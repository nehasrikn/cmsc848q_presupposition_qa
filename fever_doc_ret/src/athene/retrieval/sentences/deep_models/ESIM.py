...
class ESIM:
    ...
    def __init__(self, optimizer=tf.compat.v1.train.AdamOptimizer, h_max_length=10, s_max_length=40, learning_rate=0.0001,
    ...
    def gru_cell(self, num_neuron):
        ...
        if self.dropout_rate:
            gru = tf.compat.v1.nn.rnn_cell.DropoutWrapper(gru, input_keep_prob=self.keep_prob, output_keep_prob=self.keep_prob)
        return gru
    ...
    def _bidirectional_rnn(self, inputs, inputs_length, num_units, scope=None):
        ...
        with tf.compat.v1.variable_scope(scope or 'birnn'):
            ...
            ((fw_outputs, bw_outputs), (fw_states, bw_states)) = tf.compat.v1.nn.bidirectional_dynamic_rnn(rnn_cells, rnn_cells,
    ...
    def _mask_3d(self, inputs, sentence_lengths, mask_value, dimension=2):
        ...
        time_steps1 = tf.shape(input=inputs)[1]
        time_steps2 = tf.shape(input=inputs)[2]
        ...
        masked = tf.compat.v1.where(mask_3d, inputs, pad_values)

        if dimension == 1:
            masked = tf.transpose(a=masked, perm=[0, 2, 1])
        return masked

    def _inter_atten(self, claim, sent, claim_lengths, sent_lengths):

        with tf.compat.v1.variable_scope('inter-attention'):
            sent = tf.transpose(a=sent, perm=[0, 2, 1])
            attention = tf.matmul(claim, sent)

            masked = self._mask_3d(attention, sent_lengths, -np.inf)
            att_sent1 = self._atten_softmax3d(masked)

            att_transpose = tf.transpose(a=attention, perm=[0, 2, 1])
    ...
    def mlp(self, outputs):
        ...
        outputs = tf.compat.v1.layers.dense(outputs, 256, activation=tf.nn.tanh, kernel_initializer=self.initializer)
        if self.dropout_rate:
            outputs = tf.compat.v1.layers.dropout(outputs, rate=self.dropout_rate, training=self._training)
    ...
    def _construct_graph(self):
        ...
        X_h = tf.compat.v1.placeholder(tf.int32, shape=[None, self.h_max_length], name="X_heads")
        X_s = tf.compat.v1.placeholder(tf.int32, shape=[None, self.s_max_length], name="X_sents")
        X_h_length = tf.compat.v1.placeholder(tf.int32, shape=[None, ], name="X_h_lengths")
        X_s_length = tf.compat.v1.placeholder(tf.int32, shape=[None, ], name="X_s_lengths")
        ...
        with tf.compat.v1.variable_scope("embedding_lookup"):
            ...
            embed_h = tf.nn.embedding_lookup(params=embedding, ids=X_h)
            embed_s = tf.nn.embedding_lookup(params=embedding, ids=X_s)

        if self.share_rnn:
            with tf.compat.v1.variable_scope("encode_rnn", reuse=tf.compat.v1.AUTO_REUSE):
        ...
        if self.share_rnn:
            with tf.compat.v1.variable_scope("infer_rnn", reuse=tf.compat.v1.AUTO_REUSE):
        ...
        claim_sum = tf.reduce_sum(input_tensor=h_infer, axis=1)
        claim_mask = tf.cast(tf.sequence_mask(X_h_length), tf.float32)
        claim_ave = tf.compat.v1.div(claim_sum, tf.reduce_sum(input_tensor=claim_mask, axis=1, keepdims=True))
        claim_max = tf.reduce_max(input_tensor=h_infer, axis=1)

        sent_sum = tf.reduce_sum(input_tensor=s_infer, axis=1)
        sent_mask = tf.cast(tf.sequence_mask(X_s_length), tf.float32)
        sent_ave = tf.compat.v1.div(sent_sum, tf.reduce_sum(input_tensor=sent_mask, axis=1, keepdims=True))
        sent_max = tf.reduce_max(input_tensor=s_infer, axis=1)

        v = tf.concat([claim_ave, claim_max, sent_ave, sent_max], axis=1)

        dense_output = self.mlp(v)

        scores = tf.compat.v1.layers.dense(dense_output, 1)

        pos = tf.strided_slice(scores, [0], [self.batch_size], [2])
        neg = tf.strided_slice(scores, [1], [self.batch_size], [2])
        loss = tf.reduce_mean(input_tensor=tf.maximum(0.0, 1.0 + neg - pos))
