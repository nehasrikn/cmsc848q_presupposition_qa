...
class BiLSTM(LSTM):
    ...
    def _share_lstm(self, inputs, sequence_length):
        ...
        if self.lstm_layers == 1:
            ...
            rnn_cells_fw = tf.compat.v1.nn.rnn_cell.MultiRNNCell([self.lstm_cell(i) for i in self.num_neurons[:self.lstm_layers]])
            rnn_cells_bw = tf.compat.v1.nn.rnn_cell.MultiRNNCell([self.lstm_cell(i) for i in self.num_neurons[:self.lstm_layers]])

        inputs = tf.cast(inputs, tf.float32)
        outputs, states = tf.compat.v1.nn.bidirectional_dynamic_rnn(rnn_cells_fw, rnn_cells_bw, inputs=inputs, dtype=tf.float32,
                                                          sequence_length=sequence_length)

        if self.average_pooling:
            output_fw = tf.reduce_mean(input_tensor=outputs[0], axis=1, name="average_fw")
            output_bw = tf.reduce_mean(input_tensor=outputs[1], axis=1, name="average_bw")
    ...
    def _separate_lstm(self, heads_inputs, body_inputs, heads_length, bodies_length):
        ...
        if self.lstm_layers == 1:
            ...
            heads_rnn_cells_fw = tf.compat.v1.nn.rnn_cell.MultiRNNCell(
                [self.lstm_cell(i) for i in self.num_neurons[:self.lstm_layers]])
            heads_rnn_cells_bw = tf.compat.v1.nn.rnn_cell.MultiRNNCell(
                [self.lstm_cell(i) for i in self.num_neurons[:self.lstm_layers]])
            bodies_rnn_cells_fw = tf.compat.v1.nn.rnn_cell.MultiRNNCell(
                [self.lstm_cell(i) for i in self.num_neurons[:self.lstm_layers]])
            bodies_rnn_cells_bw = tf.compat.v1.nn.rnn_cell.MultiRNNCell(
                [self.lstm_cell(i) for i in self.num_neurons[:self.lstm_layers]])
        ...
        with tf.compat.v1.variable_scope("rnn_heads"):
            heads_outputs, heads_states = tf.compat.v1.nn.bidirectional_dynamic_rnn(heads_rnn_cells_fw, heads_rnn_cells_bw,
                                                                          inputs=heads_inputs, dtype=tf.float32,
                                                                          sequence_length=heads_length)
        with tf.compat.v1.variable_scope("rnn_bodies"):
            bodies_outputs, bodies_states = tf.compat.v1.nn.bidirectional_dynamic_rnn(bodies_rnn_cells_fw, bodies_rnn_cells_bw,
        ...
        if self.average_pooling:
            head_output_fw = tf.reduce_mean(input_tensor=heads_outputs[0], axis=1, name="avearge_pooling_heads_fw")
            head_output_bw = tf.reduce_mean(input_tensor=heads_outputs[1], axis=1, name="avearge_pooling_heads_bw")
            body_output_fw = tf.reduce_mean(input_tensor=bodies_outputs[0], axis=1, name="average_pooling_bodies_fw")
            body_output_bw = tf.reduce_mean(input_tensor=bodies_outputs[1], axis=1, name="average_pooling_bodies_bw")
