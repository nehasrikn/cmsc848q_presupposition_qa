...
These functions are modified based on tf.contrib.copy_graph
to support making a freezed tensorflow hub model become trainable again.
...
def copy_variable_to_graph(org_instance, to_graph, trainable=True, scope=''):
  ...
  Args:
    ...
    trainable: Should the new variable trainable or not
...
def copy_op_to_graph(org_instance, to_graph, variables, scope=''):
  ...
  if isinstance(org_instance, ops.Tensor):
    ...
    new_node_def = tf.compat.v1.NodeDef(name=new_name,
                              op=op.node_def.op,
