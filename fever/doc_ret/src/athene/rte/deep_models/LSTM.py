...
class LSTM(BaseEstimator, ClassifierMixin):
    ...
    def _share_lstm(self, inputs, sequence_length):
        ...
        if self.lstm_layers == 1:
            ...
            rnn_cells = tf.compat.v1.nn.rnn_cell.MultiRNNCell([self.lstm_cell(i) for i in self.num_neurons[:self.lstm_layers]])

        outputs, state = tf.compat.v1.nn.dynamic_rnn(rnn_cells, inputs=inputs, dtype=tf.float32, sequence_length=sequence_length)
        if self.average_pooling:
            output = tf.reduce_mean(input_tensor=outputs, axis=1, name="average_pooling")
    ...
    def _separate_lstm(self, heads_inputs, body_inputs, heads_length, bodies_length):
        ...
        if self.lstm_layers == 1:
            ...
            heads_rnn_cells = tf.compat.v1.nn.rnn_cell.MultiRNNCell(
                [self.lstm_cell(i) for i in self.num_neurons[:self.lstm_layers]])
            bodies_rnn_cells = tf.compat.v1.nn.rnn_cell.MultiRNNCell(
                [self.lstm_cell(i) for i in self.num_neurons[:self.lstm_layers]])

        with tf.compat.v1.variable_scope("rnn_heads"):
            heads_outputs, heads_states = tf.compat.v1.nn.dynamic_rnn(heads_rnn_cells, inputs=heads_inputs, dtype=tf.float32,
                                                            sequence_length=heads_length)
        with tf.compat.v1.variable_scope("rnn_bodies"):
            bodies_outputs, bodies_states = tf.compat.v1.nn.dynamic_rnn(bodies_rnn_cells, inputs=body_inputs, dtype=tf.float32,
                                                              sequence_length=bodies_length)

        if self.average_pooling:
            head_output = tf.reduce_mean(input_tensor=heads_outputs, axis=1, name="avearge_pooling_heads")
            body_output = tf.reduce_mean(input_tensor=bodies_outputs, axis=1, name="average_pooling_bodies")
        else:
            if self.lstm_layers == 1:
    ...
    def _dnn(self, head_inputs, body_inputs, heads_length, bodies_length):
        ...
        if self.share_parameters:
            with tf.compat.v1.variable_scope("lstm_lower") as scope:
        ...
        for layer in range(self.mlp_layers):
            ...
            if self.dropout_rate:
                inputs = tf.compat.v1.layers.dropout(inputs, rate=self.dropout_rate, training=self._training)
            inputs = tf.compat.v1.layers.dense(inputs, self.num_neurons[self.lstm_layers + layer], activation=self.activation,
                                     kernel_initializer=self.initializer,
                                     name="hidden{}".format(layer + 1))

            if self.batch_norm_momentum:
                inputs = tf.compat.v1.layers.batch_normalization(inputs, momentum=self.batch_norm_momentum,
    ...
    def _construct_graph(self, n_outputs, weights):
        ...
        if self.randome_state:
            tf.compat.v1.set_random_seed(self.randome_state)
            np.random.seed(self.randome_state)

        X_heads = tf.compat.v1.placeholder(tf.int32, shape=[None, self.h_max_length], name="X_heads")
        X_bodies = tf.compat.v1.placeholder(tf.int32, shape=[None, self.b_max_length], name="X_bodies")
        X_head_length = tf.compat.v1.placeholder(tf.int32, shape=[None], name="head_length")
        X_body_length = tf.compat.v1.placeholder(tf.int32, shape=[None], name="body_length")
        ...
        logits = tf.compat.v1.layers.dense(pre_output, n_outputs, kernel_initializer=he_init, name="logits")
        ...
        if self.tensorboard_logdir:
            ...
            file_writer = tf.compat.v1.summary.FileWriter(tb_logdir, tf.compat.v1.get_default_graph())
    ...
    def fit(self, X, y, valid_X=None, y_valid=None):
        ...
        self._session = tf.compat.v1.Session(graph=self._graph, config=tf.compat.v1.ConfigProto(gpu_options=gpu_options))
