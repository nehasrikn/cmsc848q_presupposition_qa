...
class ESIM(BaseEstimator, ClassifierMixin):
    ...
    def _multi_rnn(self, heads_embeddings, bodies_embeddings, h_sent_sizes, b_sent_sizes, scope=None, epsilon=1e-12):
        ...
        with tf.compat.v1.variable_scope(scope or "ESIM_rnn"):
            ...
            with tf.compat.v1.variable_scope("alignment"):
                ...
                sum_bodies_encoded = tf.reduce_sum(input_tensor=bodies_encoded, axis=2)
                # batch_size * encode_size
                sum_heads_encoded = tf.reduce_sum(input_tensor=heads_encoded, axis=1)
                # batch_size * sents * 1
    ...
    def _construct_graph(self):
        ...
        X_heads_fasttext = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, None, None, dim_fasttext],
                                          name="X_heads_fasttext")
        X_bodies_fasttext = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, None, None, dim_fasttext],
