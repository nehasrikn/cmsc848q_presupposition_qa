...
class ESIM(BaseEstimator, ClassifierMixin):
    ...
    def lstm_cell(self, hidden_size):
        lstm = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_size)
        if self.dropout_rate:
            lstm = tf.compat.v1.nn.rnn_cell.DropoutWrapper(lstm, input_keep_prob=self.keep_prob, output_keep_prob=self.keep_prob)
        return lstm

    def gru_cell(self, num_neuron):

        gru = tf.compat.v1.nn.rnn_cell.GRUCell(num_neuron)
        if self.dropout_rate:
    ...
    def _bidirectional_rnn(self, inputs, inputs_length, num_units, scope=None):
        ...
        with tf.compat.v1.variable_scope(scope or 'birnn'):
            ...
            if self.lstm_layers == 1:
                ...
                rnn_cells_fw = tf.compat.v1.nn.rnn_cell.MultiRNNCell([self.lstm_cell(n) for n in num_units])
                rnn_cells_bw = tf.compat.v1.nn.rnn_cell.MultiRNNCell([self.lstm_cell(n) for n in num_units])

            ((fw_outputs, bw_outputs), (fw_states, bw_states)) = tf.compat.v1.nn.bidirectional_dynamic_rnn(rnn_cells_fw,
                                                                                                 rnn_cells_bw, inputs,
                                                                                                 inputs_length,
                                                                                                 dtype=tf.float32)
    ...
    def _esim(self, head_output_steps, body_output_steps, head_sent_sizes, body_sent_sizes):
        ...
        :return:
        """
        with tf.compat.v1.variable_scope("ESIM_single_sentence_pair", reuse=True):
            ...
            self.softmax_matrix_head = soft_attention_matrix_for_head
            # batch_size * sents * body_words * head_words
            soft_attention_matrix_for_body = tf.nn.softmax(tf.transpose(a=attention_matrix, perm=[0, 1, 3, 2]), 3)
            self.softmax_matrix_body = soft_attention_matrix_for_body
            ...
            batch_size, num_sents, max_head_words, embed_size = tf.unstack(tf.shape(input=head_attended))
            ...
            head_masks_per_word = tf.compat.v1.where(head_seq_masks, tf.ones_like(head_seq_masks, dtype=tf.float32),
            ...
            _, _, max_body_words, _ = tf.unstack(tf.shape(input=body_attended))
            ...
            body_masks_per_word = tf.compat.v1.where(body_seq_masks, tf.ones_like(body_seq_masks, dtype=tf.float32),
    ...
    def _trainable_alignment(self, sum_heads_encoded, sum_bodies_encoded, encode_size, projection_size=100):
        ...
        """
        with tf.compat.v1.variable_scope("trainable_alignment") as scope:
            batch_size, sents_size, _ = tf.unstack(tf.shape(input=sum_bodies_encoded))
            ...
            vector_attn = tf.reduce_sum(
                input_tensor=tf.multiply(tf.nn.l2_normalize(bodies_projection, 2), tf.nn.l2_normalize(heads_projection, 2)), axis=2,
                keepdims=True)
        return vector_attn
    ...
    def _align_without_softmax(self, sum_heads_encoded, sum_bodies_encoded):
        ...
        vector_attn = tf.reduce_sum(
            input_tensor=tf.multiply(tf.nn.l2_normalize(sum_bodies_encoded, 2), tf.nn.l2_normalize(_tmp_heads_encoded, 2)), axis=2,
            keepdims=True)
        return vector_attn
    ...
    def _multi_rnn(self, heads_embeddings, bodies_embeddings, h_sent_sizes, b_sent_sizes, scope=None, epsilon=1e-12):
        ...
        :return:
        """
        with tf.compat.v1.variable_scope(scope or "ESIM_rnn"):
            with tf.compat.v1.variable_scope("local_inference_modelling") as scope:
                (batch_size, body_sent_size, body_word_size, embed_size) = tf.unstack(tf.shape(input=bodies_embeddings))
            ...
            with tf.compat.v1.variable_scope("inference_composition") as scope:
                attended_size = 4 * encode_size
                (batch_size, b_sent_size, h_word_size, _) = tf.unstack(tf.shape(input=heads_attended))
                (_, _, b_word_size, _) = tf.unstack(tf.shape(input=bodies_attended))
                ...
                flat_heads_mean = tf.reduce_sum(input_tensor=flat_heads_outputs, axis=1) / tf.maximum(
                ...
                #                            "flat_heads_mean:\n", summarize=80000)
                flat_heads_max = tf.reduce_max(input_tensor=flat_heads_outputs, axis=1)
                # flat_heads_max = tf.Print(flat_heads_max, [flat_heads_max, tf.shape(flat_heads_max)],
                #                           "flat_heads_max:\n", summarize=80000)
                flat_bodies_mean = tf.reduce_sum(input_tensor=flat_bodies_outputs, axis=1) / tf.maximum(
                ...
                #                             "flat_bodies_mean:\n", summarize=80000)
                flat_bodies_max = tf.reduce_max(input_tensor=flat_bodies_outputs, axis=1)
            ...
            with tf.compat.v1.variable_scope("alignment"):
                # without alignment
                ...
                output_mean = tf.reduce_mean(input_tensor=output_concat, axis=1)
                output_max = tf.reduce_max(input_tensor=output_concat, axis=1)
    ...
    def _add_embedding(self, inputs):
        embedding = tf.compat.v1.get_variable(initializer=self.embedding, dtype=tf.float32, trainable=self.trainable,
                                    name="word_embeddings")
        inputs_embedded = tf.nn.embedding_lookup(params=embedding, ids=inputs)
        return inputs_embedded

    def _ann(self, head_inputs, body_inputs, h_sent_sizes, b_sent_sizes):

        with tf.compat.v1.variable_scope("embedding_lookup") as scope:
            heads_embeddings = self._add_embedding(head_inputs)
            scope.reuse_variables()
            body_embeddings = self._add_embedding(body_inputs)
        ...
        for layer in range(self.mlp_layers):
            ...
            if self.dropout_rate:
                output = tf.compat.v1.layers.dropout(output, rate=self.dropout_rate, training=self._training)
            output = tf.compat.v1.layers.dense(output, self.num_neurons[n_birnn + layer], activation=self._activation,
    ...
    def _construct_graph(self):
        ...
        if self._initializer is None:
            if self.initializer == 'he':
                self._initializer = tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0)
        ...
        X_heads = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None, None, None], name="X_heads")
        X_bodies = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None, None, None], name="X_bodies")
        X_head_sizes = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None], name="head_sizes")
        X_body_sizes = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None], name="body_sizes")
        X_head_sent_sizes = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None, None], name="head_sent_sizes")
        X_body_sent_sizes = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None, None], name="body_sent_sizes")
        ...
        if self.dropout_rate:
            ...
            self.keep_prob = tf.cond(pred=self._training, true_fn=lambda: tf.constant(1 - self.dropout_rate),
        ...
        correct = tf.nn.in_top_k(predictions=logits, targets=y_, k=1)
        accuracy = tf.reduce_mean(input_tensor=tf.cast(correct, tf.float32), name="accuracy")
        _, predicts = tf.nn.top_k(logits, k=1, sorted=False)
        confusion_matrix = tf.math.confusion_matrix(labels=y_, predictions=predicts, num_classes=self.n_outputs, name="confusion_matrix")
        ...
        if self.tensorboard_logdir:
            ...
            cost_summary = tf.compat.v1.summary.scalar("validation_loss", loss)
            acc_summary = tf.compat.v1.summary.scalar("validation_accuracy", accuracy)
            merged_summary = tf.compat.v1.summary.merge_all()
            file_writer = tf.compat.v1.summary.FileWriter(
                tb_logdir, tf.compat.v1.get_default_graph())
    ...
    def predict_proba(self, X_dict, restore_param_required=True):
        ...
        for (pred_h_batch, pred_b_batch, pred_h_sizes_batch, pred_b_sizes_batch, pred_h_sent_sizes_batch,
            ...
            predictions_batch, attention_matrix, softmax_matrix_head, softmax_matrix_body = self._session.run(
                ...
                feed_dict={self._X_head: pred_h_batch,
    ...
    def restore_model(self, path):
        ...
        with self._graph.as_default():
            ...
            with tf.compat.v1.variable_scope("embedding_lookup", reuse=True):
                v = tf.compat.v1.get_variable("word_embeddings")
